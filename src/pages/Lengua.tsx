...
export const Lengua = () => {
  ...
  /* ---------------------------------- Hook ---------------------------------- */
  const { lenguas, updateLengua, addLengua, removeLengua } = useGet();
  ...
  const onSubmit = (values: any) => {
    if (isEdit) {
      updateLengua({ id: selectItem.id, ...values }).then(() => {
        setIsEdit(false);
        setSelectItem(null);
        form.resetFields();
      });
    } else {
      addLengua(values).then(() => {
        form.resetFields();
      });
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4">
      <h2 className="font-semibold text-blue-500 text-xl">Lengua</h2>
      <Form
        layout="vertical"
